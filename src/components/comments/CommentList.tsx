    ...
    <div className='my-10'>
      {comments?.data && comments?.data.length > 0 ? (
        comments.data.map(comment => (
          <div
            key={comment.id}
            className='flex items-center space-x-4 text-sm text-gray-500 border-b border-gray-100 pb-8'
          >
            <div>
              <img
        ...
        <div className='p-4 border border-gray-200 rounded-md text-gray-400'>댓글이 없습니다.</div>
      )}
    </div>
  )
}
