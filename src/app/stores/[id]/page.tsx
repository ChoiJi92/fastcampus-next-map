...
export default function StorePage({ params }: { params: { id: string } }) {
  ...
  const {
    ...
    isSuccess,
    isError,
  } = useQuery({ queryKey: [`store-${id}`], queryFn: fetchStore, enabled: !!id, refetchOnWindowFocus: false })
  ...
  return (
    <>
        ...
        <div className='md:flex justify-between items-center py-4 md:py-0'>
          ...
          {status === 'authenticated' && store && (
            <div className='flex items-center gap-4 px-4 py-3'>
              ...
              <button type='button' onClick={handleDelete} className='underline hover:text-gray-400 text-sm'>
                삭제
              </button>
            </div>
          )}
        ...
        <div className='mt-6 border-t border-gray-100'>
          <dl className='divide-y divide-gray-100'>
            ...
            <div className='px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0'>
              ...
              <dd className='mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0'>{store?.foodCertifyName}</dd>
            </div>
            <div className='px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0'>
              <dt className='text-sm font-medium leading-6 text-gray-900'>업종명</dt>
              <dd className='mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0'>{store?.storeType}</dd>
            </div>
          </dl>
        </div>
      </div>
      {isSuccess && (
        <>
          <div className='w-full mb-20 max-w-5xl mx-auto max-h-[600px] overflow-hidden'>
